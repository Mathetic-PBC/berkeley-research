...
A direct Storage PUT rejection now identifies the relative filename, size, HTTP status (for unknown errors), and an actionable size/authentication/rate-limit/provider explanation. For example, TutorTrace's `raw_telemetry/deployment_1.json` is about 62.46 MiB, which cannot pass a 50 MB global limit even when the dataset bucket permits 1 GiB. Raise both applicable limits to at least the largest file size on a supported plan, or use the installed local folder importer. No automatic plan upgrade or cloud security-policy change is performed.
