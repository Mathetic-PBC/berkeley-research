...
The reader gives a name, a year, a major, and a choice of how they want things explained, one screen each. Then they hand over the paper as a PDF, optionally a project page and a code repository, and say how familiar they are with the paper themselves. Next they install Engelbart on their machine: they pick their computer and its chip, and get two terminal instructions one at a time, each stating the keys to press as key chips and offering the command to copy; the second installs Claude Code if the machine has none, installs Engelbart, connects their account with a short code, and tells the installer not to open its own setup page. Then a short brainstorm: two to four grounded possibilities, at most one preference question per turn, and usually three complementary preference signals with a hard cap of three user responses. It learns an interest for Direction, using the paper's concrete things and the reader’s stated background, without planning the project. Next come the topic questions: two to four areas the paper turns on, a familiarity slider and one question each, graded silently against a sample answer, with one follow-up when the grade disagrees: a new question, written from what the reader just said, at the level the grade found. While it waits the slider is locked, and it is stored, so a reload or a walk to another area and back finds the same question. The card is the whole conversation: there is no free-text box beside it, the opening card comes without a preamble, and a card that has been answered stays on the thread as a card with the answers marked. Then the list of those things — datasets, code, tools, demos, simulations, instruments — fitted to the reader; a row whose stand-ins exist folds them behind a "simpler" toggle, picking a row shows what it is and where it lives, and asking about any of it is the page's own Ask about this; they pick one. Then one direction, proposed rather than chosen from three, with a box to say what should change; then three subgoals the same way; then todos for the first subgoal only, editable, with a name. The last screen walks them the same way to a new terminal, `claude`, Claude Code's own trust-this-folder question, and `/bart`, which picks the project up. Selecting or clicking any text on any step offers Ask about this in the left margin. From the paper step on, a copy of the Explanations slider sits in the top right: drag it and press Regenerate, and what is on the screen is rewritten at that register in one Haiku call, with the profile's depth following so what comes next is written there too. Deleting a todo takes two presses. Pressing ⏎ anywhere on the page that is not a text box presses the step's own button. The browser is only a mirror of the stored setup, so closing the tab and coming back later returns them to the furthest step they reached with everything they typed still there, including the brainstorm.

## What runs while the reader keeps going

Handing over the paper starts two readings side by side, neither of which the reader waits for. The first produces the topic areas and their question ladders. The second hunts for the concrete inputs and outputs of the work — the datasets, tasks and apparatus, codebooks, experimental paradigms, models, simulations, analysis pipelines, surveys and coding schemes, libraries, source code — at most five, each a specific building block of this paper rather than a general tool it used, written in the paper's own register, each with links the model searched the web for and the server then checked, dropping any that answer "gone". A short projection of that list, names and one-liners only, is what the brainstorm is given. When the last topic question is answered, the grades are compiled into one assessment without a model call, and a third process starts: it takes the hunt's list and the assessment, decides where the locus of problem solving lies for this reader and which knowledge is sticky, adds beginner stand-ins as children only where the sticky part of an asset is one the grades show the reader lacks (the reader builds with Claude Code, so code and repositories are usually within reach whatever the grades say; a mathematical simulation may not be), and rewrites every description at the reader's register. If the hunt is still out, resource fitting waits. Brainstorm readiness depends only on human context: the student usually supplies what interests them, what they want to do with it, and what they want to discover, change, or compare; unusually specific intent can finish earlier, and a third nonempty user response always ends it. Polling or reopening a settled conversation never asks another question. Continue or Skip from Brainstorm goes to Topics. Completing Topics starts resource fitting and opens resource selection; Direction follows the selected resource.
...
| The hunt for the paper's things failed, or fitting them to the reader did | The brainstorm shows the error with a retry; the plan is offered only once the fitted list exists and the model says the reader is ready |
...
one saved stage. Direction checks its resource first; each plan then drafts from existing Analysis and reviews. A rejected first draft gets one correction
...
Resource selection is headed “Select which resource to start with” and explains
that the dataset, code, or demo becomes the first project's starting point. The
first resource's details and smaller alternatives open by default; no resource is
selected until the reader picks one, and the expansion can be closed.

Apply `20260908060000_brainstorm_before_topics.sql` with this order change. It swaps
