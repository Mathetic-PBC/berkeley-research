...
A workspace refresh can automatically try to start a deterministically detected web app when its preview state is unconfigured, ready, or stale and no build is active or installation/setup blocker remains. This is not restricted to commands previously run successfully. Automatic startup excludes model guesses and honors an explicit saved `autostart: false`. Some comments still describe click-only startup; the current callers and guards implement the behavior above. [Automatic startup caller][preview-auto], [Startup guards][preview-show].
