...
test("browser → CLI → /bart browser → Claude context", async ({ page }) => {
  ...
  try {
    ...
    await expect(page.locator(".dataset-preview tbody tr").first()).toHaveText("editsession-one");
    ...
    await page.getByLabel("Upload dataset", {exact:true}).setInputFiles({name:"actual-protocol.tsv",mimeType:"text/tab-separated-values",buffer:Buffer.from("event\tsession_id\nverified-upload\treal-session\n")});
    await expect(page.getByRole("heading",{name:"actual-protocol.tsv",exact:true})).toBeVisible();
    await expect(page.locator(".dataset-preview")).toContainText("verified-upload");
    const resources = await page.evaluate(()=>window.engelbart.store.get().project);
    expect(resources.activeDatasetId).not.toBe(dataset.id);
    expect(resources.resources.find(r=>r.id===dataset.id).provenance.fallbackOf.kind).toBe("synthetic_fallback");
    const active = resources.resources.find(r=>r.id===resources.activeDatasetId);
    expect(active.status).toBe("ready");
    expect(active.access.originalFile).toContain("actual-protocol.tsv");
    expect(active.provenance.replaces[0].id).toBe(dataset.id);
    await page.reload();
    await expect(page.getByRole("tab", { name: "Dataset", exact: true })).toBeVisible();
    await page.getByRole("tab", { name: "Dataset", exact: true }).click();
    await expect(page.locator(".resource-detail")).toContainText("actual-protocol.tsv");
