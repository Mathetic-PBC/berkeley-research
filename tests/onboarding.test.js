...
test("migrated sessions keep their current step, interest, and calibration answers", async () => {
  for (const [step, assessment, expected] of [[8, null, OB.STEP.topics], [7, { areas: [] }, OB.STEP.brainstorm]]) {
    ...
    const count = db.tables.engelbart_onboarding_calibrations.length;
    const out = await OB.open(USER, {}, db.options);
    assert.equal(out.onboarding.step, expected);
    assert.equal(out.onboarding.interest, "Compare repeated failed runs");
