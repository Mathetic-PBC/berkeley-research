...
test('replacing an article or dataset clears derived state and rejects an older brainstorm completion',async()=>{
 ...
 try {
  ...
  const grounding={contribution:'Prior contribution',evidence:[{kind:'method',claim:'Prior method',quote:'Prior quote',location:'Section 2'}]};
  await db.query("insert into engelbart_onboardings(id,user_id,source_article,analysis_status,analysis) values($1,$2,$3,'done',$4)",[id,user,{text:'First article'},{title:'First',grounding}]);
  await db.query("update engelbart_onboardings set analysis=$1 where id=$2",[{title:'Reread'},id]);
  assert.deepEqual((await db.query('select analysis from engelbart_onboardings where id=$1',[id])).rows[0].analysis.grounding,grounding);
  ...
  await db.query("update engelbart_onboardings set source_article=$1,analysis=null where id=$2",[{text:'Replacement article'},id]);
  ...
  await db.query("update engelbart_onboardings set analysis=$1,analysis_status='done' where id=$2",[{title:'Replacement',grounding},id]);
  await db.query('update engelbart_onboardings set dataset_resource=$1,analysis=null where id=$2',[{id:'new-dataset'},id]);
  const datasetRow=(await db.query('select analysis,analysis_status,source_revision from engelbart_onboardings where id=$1',[id])).rows[0];
  assert.equal(datasetRow.analysis_status,'none');assert.equal(datasetRow.analysis,null);assert.equal(datasetRow.source_revision,2);
