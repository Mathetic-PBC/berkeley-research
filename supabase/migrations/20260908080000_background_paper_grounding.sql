-- Reuse planning JSON for one paper-scoped lifecycle; the evidence stays in analysis.
create or replace function public.engelbart_valid_grounding(g jsonb)
returns boolean language sql immutable set search_path=public as $$
  select coalesce(jsonb_typeof(g->'contribution')='string' and btrim(regexp_replace(g->>'contribution','[[:space:]]+',' ','g'))<>''
    and exists(select 1 from jsonb_array_elements(case when jsonb_typeof(g->'evidence')='array' then g->'evidence' else '[]'::jsonb end)
      with ordinality as evidence(e,n) where n<=10 and e->>'kind' in ('method','experiment','artifact')
      and jsonb_typeof(e->'claim')='string' and btrim(regexp_replace(e->>'claim','[[:space:]]+',' ','g'))<>''
      and jsonb_typeof(e->'quote')='string' and btrim(regexp_replace(e->>'quote','[[:space:]]+',' ','g'))<>''
      and jsonb_typeof(e->'location')='string' and btrim(regexp_replace(e->>'location','[[:space:]]+',' ','g'))<>''),false);
$$;
...
begin
 ...
 job := jsonb_build_object('status','running','started_at',now(),'lease_until',now()+interval '130 seconds','token',p_token,'error',null);
...
begin
 if old.paper_id is distinct from new.paper_id then
   ...
   -- Re-reading lightweight Analysis for the SAME PDF cannot erase its evidence.
   new.analysis := coalesce(new.analysis,'{}'::jsonb) || jsonb_build_object('grounding',old.analysis->'grounding');
 end if;
 return new;
end $$;
create trigger engelbart_grounding_paper_change before update of paper_id,analysis on public.engelbart_onboardings
for each row execute function public.engelbart_clear_grounding_for_paper();
