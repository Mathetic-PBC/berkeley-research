    ...
    A.analysis = function (ctx, row, cals, body) {
      ...
      return patchRow(ctx, row, { analysis_status: "running", analysis_started_at: now(), analysis_error: "" }, "mark the reading running")
        ...
        .then(function () {
          ...
          var content = paperPrefix().concat([{ type: "text", text: promptText("analyzePrompt", { familiarity: PR.FAMILIARITY[Number(row.paper_familiarity) || 0], depth: PR.depthOf(row.depth) || PR.DEPTHS[0], urls: pages.length ? pages.map(function (p) { return p.url + "\n(" + p.chars + " characters of fetched page text)"; }).join("\n\n") : "" }, knobs) }]);
