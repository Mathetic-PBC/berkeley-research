...
function readerBlock(reader) {
  ...
  if (reader.paper_familiarity != null && FAMILIARITY[Number(reader.paper_familiarity)]) out.push("Self-reported paper familiarity (not a grade): " + FAMILIARITY[Number(reader.paper_familiarity)].desc);
  if (reader.chosen_depth && reader.chosen_depth !== reader.depth) out.push("Chosen explanation level: " + reader.chosen_depth + ".");
  if (knowledge.length) {
...
function todosPrompt({ reader, paper, direction, subgoal, resources }) {
  ...
  const sg = subgoal || {};
  return [
    "Engelbart prepares the selected paper and dataset as project resources during local handoff. Downloading, unzipping, finding files, extracting paper text, and inspecting schema are infrastructure prerequisites, never human TODOs or subgoals. Start with meaningful research interaction. Do not replace a selected real dataset with a synthetic one merely to avoid preparation; access or license blockers must remain explicit.",
    ...readerBlock(reader), ...resourcesBlock(resources), "",
    `They are building on "${paper.title}" -- ${paper.one_liner}`,
...
function assessmentBlock(assessment) {
  ...
  for (const x of areas) {
    const r = (x.graded_level != null ? rung(x.graded_level) : null);
    const said = Array.isArray(x.answers) && x.answers.length ? ` They wrote: "${String(x.answers[x.answers.length - 1]).slice(0, 240)}"` : "";
    out.push(`- ${x.area}: rated themselves ${x.self_level != null && rung(x.self_level) ? rung(x.self_level).phrase : "?"}; graded ${r ? r.phrase + " (" + r.level + ")" : "ungraded"}${x.rationale ? " -- " + x.rationale : ""}.${said}`);
  }
  return out;
}
...
function levelPrompt({ reader, assessment, assets, interest }) {
  return [
    ...
    JSON.stringify(require('./dataset-collections').compact({assets}), null, 0),
    "",
...
const J = (v) => JSON.stringify(require('./dataset-collections').compact(v == null ? {} : v), null, 0);
