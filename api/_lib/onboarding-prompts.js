...
function todosPrompt({ reader, paper, direction, subgoal, resources }) {
  ...
  return [
    ...
    "",
    "Write the TODO rows for the FIRST subgoal only: two to four rows maximum, in the imperative, the smallest set of agent-executable tasks necessary to make it true, each concrete, checkable and small enough for a session. They should not have to perform prerequisite work before these rows can begin; the coding agent carries the setup burden. It may download or clone the relevant resource; inspect files, schemas, documentation, APIs, or repository structure; install dependencies; determine which parts of a large resource matter; choose a small representative example or subset; convert awkward source formats; start an existing demo; and create the minimum viewer needed to expose the result. Do not turn those into conceptual TODOs for the human such as \"understand the schema\", \"explore the dataset\", \"choose a sample\", \"read the repository\" or \"decide which variables to use\"; write instead the concrete work the coding agent can perform that makes the result available to them. Where a resource above is the right starting point, name it in the row. Do not write rows for the other subgoals.",
    "Prefer the fewest implementation steps necessary, and combine setup and plumbing operations when they do not create distinct value for the reader. The first subgoal is complete when they can open something concrete and understand, at a basic level, what resource is now in front of them. Do not build a polished application yet: a minimal functional table, plot, viewer, page, notebook output, or runnable example is enough. The result should create orientation through interaction rather than require orientation before interaction. Where a real resource is too large or expensive for the first pass, use the smallest REAL subset that preserves its meaning; use a synthetic fixture only when the real resource genuinely cannot be accessed yet.",
...
// The most question rounds a brainstorm gets: an assistant turn that carried
// a questions or focus card is one round. The server closes the conversation
// after this many whatever the model asked for.
const BRAINSTORM_ROUNDS = 2;

function roundLine(round) {
  const n = Number(round) || 0;
  if (n <= 0) return "Question rounds so far: none. This turn may ask.";
  if (n === 1) return "Question rounds so far: one. Ask a second only if their answers revealed something that would materially change the project you would propose; otherwise set `ready` true.";
  return "Question rounds so far: two, the most allowed. Do not ask again: set `ready` true, make the card `none`, and say in a sentence or two what you will plan around.";
}
...
// reader's latest turn last), round (question rounds asked so far). The
// model may say `ready` on any turn; the fitted resources are not a condition.
function brainstormPrompt({ reader, paper, assessment, brief, turns, round }) {
  ...
  return [
    ...
    "",
    "You are brainstorming with them only enough to identify a plausible project direction they would be excited to pursue: the piece of this paper worth extending or reproducing in a small first project. Optimize for information gained, not conversation length. Use the graded levels above: do not ask what the grades already answered.",
    "Ask at most one or two high-value questions in a card. Ask a second round only if the first answers revealed something that would materially change the project you would propose. Never ask a third round of brainstorming questions.",
    "They may not have opened the dataset, repository, code, model, simulation, or other resources yet. Do not ask them to choose between technical things they have not had a chance to understand. Do not ask questions whose answers the coding agent can discover from the paper, repository, dataset, documentation, or existing resources.",
    "Prefer questions they can answer from their own interests and intent: what phenomenon or behavior catches their attention; what they would be excited to see, change, compare, or make possible; who or what they care about the result being useful for; a real constraint that would materially change the project.",
    "They will build the project with Engelbart's coding agent, which writes, runs and debugs the code with them. Their programming ability is therefore not a constraint and not a question: never ask what languages they know, whether they have used an API, or whether they can code.",
    "Once you know (1) what draws them to the work and (2) one plausible outcome they would be excited to see, you know enough to continue: set `ready` true. The resources are being fitted to them in the background; do not continue asking questions merely because that is still happening.",
    roundLine(round),
    "",
    ...
    " \"ready\": true | false}",
    "",
    "Only the key for the card you name is read. `questions` is for one or two questions whose answers change what you would propose -- mcq (one answer), select_all (say so in the subtitle), free (one line), open (a paragraph); never questions you could assume the answer to. `focus` is for when what they want could be read two or three ways and which one decides everything after it; two to four options with a why each, each a reading of their intent rather than a technical choice they have not seen. There is no free-text box beside the card: a turn that asks carries a card, and a turn that is ready carries `none`.",
    ...
    "`ready` says whether you know what draws them and one plausible outcome they would be excited to see. It is not a question to them. Set it true on whichever turn you know both, and ask nothing more after that: with `ready` true the card is `none`, and `say` tells them in a sentence or two what you will plan around.",
  ].filter((line) => line !== "").join("\n");
}
...
const TEMPLATES = {
  ...
  brainstormPrompt: "{{reader_block}}{{assessment_block}}They are about to start a first project that builds on \"{{paper_title}}\" -- {{paper_one_liner}}{{brief_block}}{{transcript_block}}\nYou are brainstorming with them only enough to identify a plausible project direction they would be excited to pursue: the piece of this paper worth extending or reproducing in a small first project. Optimize for information gained, not conversation length. Use the graded levels above: do not ask what the grades already answered.\nAsk at most one or two high-value questions in a card. Ask a second round only if the first answers revealed something that would materially change the project you would propose. Never ask a third round of brainstorming questions.\nThey may not have opened the dataset, repository, code, model, simulation, or other resources yet. Do not ask them to choose between technical things they have not had a chance to understand. Do not ask questions whose answers the coding agent can discover from the paper, repository, dataset, documentation, or existing resources.\nPrefer questions they can answer from their own interests and intent: what phenomenon or behavior catches their attention; what they would be excited to see, change, compare, or make possible; who or what they care about the result being useful for; a real constraint that would materially change the project.\nThey will build the project with Engelbart's coding agent, which writes, runs and debugs the code with them. Their programming ability is therefore not a constraint and not a question: never ask what languages they know, whether they have used an API, or whether they can code.\nOnce you know (1) what draws them to the work and (2) one plausible outcome they would be excited to see, you know enough to continue: set `ready` true. The resources are being fitted to them in the background; do not continue asking questions merely because that is still happening.\n{{round_line}}\nReply with ONE JSON object and nothing else:\n{\"say\": \"<what you say to them, plain prose, at most three sentences; empty when the card says it all>\",\n \"card\": \"questions\" | \"focus\" | \"none\",\n \"questions\": {\"eyebrow\": \"<two or three words>\", \"items\": [{\"id\": \"<short slug>\", \"type\": \"mcq\" | \"select_all\" | \"free\" | \"open\", \"title\": \"<the question>\", \"subtitle\": \"<optional>\", \"options\": [{\"label\": \"<the choice>\", \"why\": \"<optional: what it buys them>\"}], \"placeholder\": \"<for free and open>\"}]},\n \"focus\": {\"title\": \"<what you are asking them to choose between>\", \"options\": [{\"label\": \"<one reading of what they could build>\", \"why\": \"<why this one>\"}]},\n \"interest\": \"<one sentence: what they seem drawn to so far, in the third person; empty if you cannot tell yet>\",\n \"ready\": true | false}\nOnly the key for the card you name is read. `questions` is for one or two questions whose answers change what you would propose -- mcq (one answer), select_all (say so in the subtitle), free (one line), open (a paragraph); never questions you could assume the answer to. `focus` is for when what they want could be read two or three ways and which one decides everything after it; two to four options with a why each, each a reading of their intent rather than a technical choice they have not seen. There is no free-text box beside the card: a turn that asks carries a card, and a turn that is ready carries `none`.{{opening_line}}\nDo not propose goals, todos, or a plan; that comes later. Write at the register above.\n`ready` says whether you know what draws them and one plausible outcome they would be excited to see. It is not a question to them. Set it true on whichever turn you know both, and ask nothing more after that: with `ready` true the card is `none`, and `say` tells them in a sentence or two what you will plan around.",
...
const LABELS = { analyzePrompt: "Read the paper", assetsPrompt: "Hunt for the paper's things", gradePrompt: "Grade an answer", followUpPrompt: "Write a follow-up", levelPrompt: "Fit the resources to the reader",
  brainstormPrompt: "Brainstorm turn", directionPrompt: "Propose a direction", subgoalsPrompt: "Break into three pieces", todosPrompt: "Write todos", askPrompt: "Answer a question about selected text", rewritePrompt: "Rewrite at another register" };
...
const DIRECTION_TASK = "Choose ONE meaningful direction for their first project: the concrete capability, question, or result they should eventually reach. Not three to pick from: the one that best fits everything above. The direction describes where the project is going, NOT everything they must already understand before beginning. They may currently know very little about the dataset, repository, model, experimental system, or terminology; that is fine. The coding agent and the subgoals that follow will bridge them from that starting point to this direction. Choose a direction that clearly follows from what interested them; uses something concrete from the paper, the thing they chose above by preference; could become a meaningful extension, comparison, reproduction, or modification; and gives them something they could eventually show the researcher. Do not turn the direction into a generic \"explore the dataset\" or \"build a dashboard\" project merely because they are a beginner. Do not assume the first implementation step is the direction itself: the next stage will construct the shortest path from their current state to this destination. It should sit where the problem solving lies for THEM, not in the part a library or the coding agent will carry.";
const SUBGOALS_TASK = "Break the direction into exactly three subgoals, in the shortest sensible order for THIS reader. Optimize for minimum prerequisite burden on the HUMAN, not merely the minimum amount of code. They may not yet have downloaded the resource, opened the repository, seen the dataset, understood its structure, run the model, or learned the terminology; do not make them do those things before implementation begins. The coding agent carries that setup and uses the artifact itself to orient them. When the project depends on an unfamiliar dataset, repository, model, simulation, instrument, codebase, or other concrete resource, prefer this progression. Subgoal 1, get the real thing into their hands: the coding agent obtains or opens the relevant resource and makes its basic structure inspectable, so the finished capability lets them look at something concrete and answer, at a basic level, what is here, what kinds of things it contains, and what one real example looks like. That may be a simple table, viewer, rendered example, plot, runnable demo, notebook output, or other appropriate surface; the agent chooses sensible defaults and a small representative subset when necessary, and they are not required to choose columns, files, examples, parameters, or methods before they have seen them. Subgoal 2, make one representative example work end-to-end: the coding agent chooses a clear representative example from the paper or resource and connects the minimum real input, operation and output path, so they can see what went in, what happened, and what came out, with enough plain-language context to understand the example; they should not yet need to design the experiment themselves. Subgoal 3, give them one meaningful thing to change, compare, test, or investigate: now that they have seen the resource and one working example, expose the smallest meaningful decision that moves toward the direction -- comparing two cases, changing one parameter, trying another input, evaluating an output, finding an unusual case, modifying one behavior. This is where they begin directing the work instead of merely becoming oriented to it. If the project does not depend on an unfamiliar external resource, adapt these roles while preserving the progression: orientation, then one concrete end-to-end example, then one meaningful manipulation. Every subgoal must still be an observable capability someone could tell you is working, not a human activity, phase, heading, topic, or unanswered decision. Reject subgoals like \"Learn the dataset\", \"Read the documentation\", \"Understand the codebase\", \"Explore possible approaches\", \"Choose an example\", \"Decide what to analyze\" or \"Study the paper\": those are either tasks the coding agent should carry or understanding that should emerge from interacting with what the agent builds. Apply this test to each: \"If the reader stopped after this subgoal, what new thing could they actually open, run, see, or do?\" If there is no concrete answer, rewrite the subgoal. Each carries a description (two sentences defining observable done conditions) and a why (one sentence explaining the dependency on what comes before).";
...
function slots(key, v) {
  ...
  if (key === "subgoalsPrompt") { const lv = v.leveled || {}, d = v.direction || {}; return { reader_block: head(readerBlock(v.reader)), paper_title: paper.title, paper_one_liner: paper.one_liner, direction_title: d.title, direction_what: d.what_you_would_make || "", first_visible: d.first_visible_result ? " First visible result: " + d.first_visible_result : "",
    ...
    task: v.previous ? "Revise the three subgoals to do what they asked. Keep what they did not object to." : SUBGOALS_TASK }; }
...
module.exports = {
  TEMPLATES, LABELS, ORDER, OVERRIDE_MAX_CHARS, BRAINSTORM_ROUNDS, slots, render, sanitizeOverrides,
